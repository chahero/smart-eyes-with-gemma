# 파일: backend/app/video_demo_main.py
# 역할: 동영상 입력용 FastAPI 서버
...
app = FastAPI(
    title="Smart Eyes with Gemma - Video Demo API",
    description="Backend server for video input analysis and voice guidance",
    version="1.0.0"
)
...
def read_root():
    ...
    return {"status": "Smart Eyes with Gemma Video Demo API is running!"}
...
# 루트 경로('/')로 접속 시, video_demo.html 파일을 반환
@app.get("/", response_class=HTMLResponse)
async def serve_video_demo():
    video_demo_path = os.path.join(frontend_dir, "video_demo.html")
    if not os.path.exists(video_demo_path):
        return HTMLResponse(
            content="""
            <html>
                <body>
                    <h1>Smart Eyes with Gemma - Video Demo</h1>
                    <p>Frontend video_demo.html not found.</p>
